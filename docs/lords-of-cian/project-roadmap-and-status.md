...
- ~~Deeper physiology treatment of Efa Gol and Pell Ostra beyond their thin `MCD-233` mentions~~
  **done, Batch 72, 2026-09-06 (`CC-130` through `CC-133`).** Sourced from the Chronicles I-VIII
...
## 7. Separate, unrelated thread — the interactive archive app

Repo `The-Reaver/My-Rivals-Distance-Archive`, not blocked on canon work and canon work not blocked on
it. As of the last check (2026-09-03): one real commit, a genuine Next.js App Router + Supabase build
with a landing page and character-index page. Both migrations implement comprehensive RLS with the
`knowledge_core` schema fully revoked from `anon`/`authenticated`. Not independently verified live —
the Supabase project is currently paused. Standing instruction: canon-writing sessions don't build the
app, but new canon material should be drafted with the eventual archive site's SEO/GEO and gamified
five-tier reader-unlock model in mind — which is where the Brain Trust blocker below comes in.

**Status: not actively worked, not urgent, unblocked from canon work.**

---

## 8. Standing blocker — real Brain Trust review

A proposed SEO/GEO/content-structuring charter for the archive site needs the operator's actual Brain
Trust review process, which requires a live device-bridge session (the Cowork app or local Claude Code,
desktop app open, `desktop-4uc2ltp` connected) — a cloud session like this one structurally cannot
reach it. A stand-in 4-seat panel was run once as a first-pass draft but is explicitly not ratified.

The real fix is written and waiting, unexecuted: `research/knowledge-home/structure-notes/
