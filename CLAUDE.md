...
`canon-ledger.json` in this folder is the authoritative canon-rules ledger for the Lords of Cian world. It is a flat list of atomic, source-cited rules (`rules`), a log of every extraction/invention pass (`batches_completed`), and a small set of still-open questions (`open_decisions`). As of 2026-08-25 it sits at `ledger_version` 2.6, 530 rules, 23 batches, zero duplicate rule IDs.
...
- The remaining Ashkeel batches: ~~the Seven High Arts as a formal system~~ **done, Batch 22, 2026-08-25 (`ASH-023` through `ASH-030`)**; ~~the collar/legal hierarchy and the Basalt Codex~~ **done, Batch 23, 2026-08-25 (`ASH-031` through `ASH-042`)**; still queued: internal geography and the merchant guilds, and named characters.
  - Both come from the same source document as Batches 19, 22, and 23: "Guild of the Extraordinary" (Google Drive fileId `1cGEqnWXfUZOGSVksys32TSQninqSI_fWZ0LC7fSUGXM`). Quirks confirmed there that will resurface in the remaining two batches:
    ...
    - The document also reuses "Cian" (the world's own name) as an in-world proper-noun component in several places (Redoubts of Cian, Apothecaries of Cian, and, before corrections, the Vapor of Cian -> now the Obsidian Vapor, and the Sanctuary of Cian -> now the Sanctuary of Ashkeel). Abad's standing call: decide per-instance, not a blanket rule; flag each occurrence with a read and let him confirm.
